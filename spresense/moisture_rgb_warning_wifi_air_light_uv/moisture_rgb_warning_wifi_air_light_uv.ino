...
// photo cell
int photocellPin = A2;     // A2 photo cell connection
int photocellReading, photocellVal;  
...
// temporal stuff
unsigned long startMillis, currentMillis, startWaterMillis, currentWaterCheckMillis;
...
void setup() {  
  ...
  // success, print IP
   ...
   startWaterMillis = millis();  //get the current "time" (actually the number of milliseconds since the program started)
...
void loop() {
 ...
 if (output_value < 30 && output_value > 5 ) {
    ...
    valve=1; 
    cmdCheckWaterTank();  // send reminder 
   } 
 if (output_value < 5 && output_value > -10 ) {
   ...
   Serial.print("hum_soil:"); Serial.print(output_value); 
   Serial.print("%"); Serial.print(" temp_out:");
   Serial.print(temp_out);  Serial.print("'C humid_out:");
   Serial.print(humid_out); Serial.print("% pump:");   Serial.print(valve);  
   Serial.print(" air_q:");  Serial.print(airqval); 
   Serial.print(" air_alarm:"); Serial.print(airqstr); 
   Serial.print(" light:");  Serial.print(photocellVal);
...
void cmdCheckWaterTank() {
   currentWaterCheckMillis = millis();
   if (output_value < 30 && valve == 1 ) {
          if (currentWaterCheckMillis - startWaterMillis >= REMINDER)  //test whether the period has elapsed
              {
                 Serial.print(currentWaterCheckMillis - startWaterMillis);
                 Serial.print("low water, send reminder"); 
                 Serial.println(output_value); 
                 digitalWrite(RELAY1,0);  
                 valve=0; 
                 valve_reminder = true;
                 startWaterMillis = currentWaterCheckMillis;
              }
          }
  }   
...
void getEnvData(){
  ...
  photocellVal = map(photocellReading, 0, 1023, 0,100); 
